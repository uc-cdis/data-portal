...
export function createSvgGraph(nodesIn, edges) {
  ...
  const minX = Math.round(
    1 / d3.extent(nodes.map((node) => node.position[0]))[0]
  );
  const minY = Math.round(
    1 / d3.extent(nodes.map((node) => node.position[1]))[0]
  );
  ...
  const graph = svg.append('g').attr('transform', `translate(0,${padding})`);
  ...
  const nodesForQuery = nodeTypes.filter(
    (nt) => !unclickableNodes.includes(nt)
  );
    ...
    .on('click', (_, d) => {
      for (let i = 0; i < nodesForQuery.length; i += 1) {
        if (d.id === nodesForQuery[i]) {
          window.open(
            window.location.href.concat('/search?node_type='.concat(d.id))
          );
          break;
        }
      }
    ...
    for (let i = 0; i < splitName.length; i += 1) {
      graph
        ...
        .attr(
          'dy',
          `${
            (0 - (splitName.length - i - 1 - nodeInfo.adjust_text_pos)) * 0.9
          }em`
        )
        .text(splitName[i]);
    }
  ...
  function positionLink(d) {
    if (d.source.fy === d.target.fy) {
      return `M${d.source.x},${d.source.y}Q${d.source.x},${
  ...
  function ticked() {
    link.attr('d', positionLink).attr('stroke-dasharray', (d) => {
      if (d.exists === 0) {
        return '5, 5';
      }
      return '0';
...
class SvgGraph extends React.Component {
  ...
  onUpdate = () => {
    ...
    if (
      ...
      this.state.edges !== this.props.edges
    ) {
