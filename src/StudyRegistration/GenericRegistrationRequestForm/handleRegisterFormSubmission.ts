        ...
        if (status === 201) {
          if (data && data.request_id) {
            ...
            Object.entries(formValues).filter(([key]) =>
              !key.includes('_doNotInclude')).forEach((entry) => {
            ...
            createKayakoTicket(
              ...
              ).then(() => setFormSubmissionStatus({ status: 'success' }),
              (err) => setFormSubmissionStatus({ status: 'error',
                text: err.message }));
          } else {
          ...
          setFormSubmissionStatus({ status: 'error', text: `Failed to create a
