...
class DataExplorerVisualizations extends React.Component {
  ...
  render() {
    const charts = this.props.arrangerData
      ? getCharts(
          this.props.arrangerData,
          this.props.dataExplorerConfig,
          this.props.sqon
        )
      : null;
    if (this.props.dataExplorerConfig.charts.fileCounts && charts) {
      charts.countItems.push({
        label: 'Files',
        value: this.state.manifestEntryCount,
      });
    }

    const toaster = (
      <Toaster
        isEnabled={this.isToasterOpen()}
        className={'data-explorer__toaster-div'}
      >
        <Button
        ...
        {this.state.exportStatus === 200 ? (
          <div className='data-explorer__toaster-text'>
            ...
            {this.state.nodeIds.length > 10000 && (
              <div>
                {' '}
                (Export/download functionality is limited to first 10,000
                records){' '}
              </div>
            )}
    ...
    return (
      <div className='data-explorer__visualizations'>
        ...
        {this.props.dataExplorerConfig.table &&
        this.props.dataExplorerConfig.table.enabled ? (
