...
describe('DataExplorerFilters', () => {
  ...
  const componentWithoutFilters = mount(<DataExplorerFilters {...props} />);
  ...
  it('shows tabs when config is provided', () => {
    ...
    expect(componentWithFilters.find('.g3-filter-group__tab').length).toBe(
  ...
  it('doesnt show tabs when config is not provided', () => {
    ...
    expect(componentWithoutFilters.find('.g3-filter-group__tab').length).toBe(
      0
    );
  });
});
