  ...
  const openFillRequestFormButton = (
    props.config.features.exportToWorkspace?.openFillRequestForm && (
      <Popover
        ...
        content={(
          <span className='discovery-popover__text'>After filling the request form, after your search selection, you would get an approval after which you can use the Gen3 Client
    to download the data from the selected studies to your local computer.
          </span>
        )}
      >
        <Button
          onClick={(props.config.features.exportToWorkspace?.fillRequestFormURL) ? () => {
            ...
            props.discovery.selectedResources.forEach((item) => {
              // Append the "id" value to combinedIds with a comma if it's not the first iteration
              if (combinedIds !== '') {
                combinedIds += ',';
              }
