...
const DiscoveryListView: React.FunctionComponent<Props> = (props: Props) => {
  ...
  const scroll = props.config.tableScrollHeight
  ...
  return (
    <Table
        ...
        props.config.features.exportToWorkspace.enabled && {
          ...
          getCheckboxProps: (record) => {
            ...
            if (props.config.features.authorization.enabled) {
              disabled =
                record[props.accessibleFieldName] !== AccessLevel.ACCESSIBLE;
            }
            // disable checkbox if there's no manifest found for this study
            const exportToWorkspaceConfig =
              props.config.features.exportToWorkspace;
            const { manifestFieldName } = exportToWorkspaceConfig;
            if (
              !record[manifestFieldName] ||
              record[manifestFieldName].length === 0
            ) {
              disabled = true;
            }
            return { disabled };
      ...
      rowClassName={(_, index) =>
        index === onHoverRowIndex
          ? 'discovery-table__row--hover'
      ...
      expandable={
        props.config.studyPreviewField && {
          // expand all rows
            ...
            const renderValue = (
              ...
              if (searchTerm) {
                ...
                const matchIndex = value
                  .toLowerCase()
                  .indexOf(props.searchTerm.toLowerCase());
                if (matchIndex === -1) {
          ...
          expandedRowClassName: (_, index) =>
            index === onHoverRowIndex
              ? 'discovery-table__expanded-row--hover'
              : 'discovery-table__expanded-row',
