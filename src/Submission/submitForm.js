...
class EnumInput extends Component {
 ...
 render() {
   const options = this.props.options.map((option, i) => ({ label: option, value: option }));
    ...
    <div>
      ...
      <AnyOfSubProps>
        {properties.map((property, i) => {
          ...
          const required_subprop = (requireds.indexOf(property) > -1);
...
class SubmitFormContainer extends Component {
 ...
 render() {
   ...
   const node_types = this.props.submission.node_types;
   const node = dictionary[this.state.chosenNode.value];
   const options = node_types.map(node_type => ({ value: node_type, label: node_type }));
