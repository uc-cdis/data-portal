...
function create_abridged_graph(nodes, edges) {
  ...
  let padding = 25, 
    ...
    legend_width=125,
    width = max_x_pos * radius * 5, 
    height = max_y_pos * radius * 3;

  let box_height = radius * 1.5
  let box_width = radius * 3
...
function add_tables(nodes, box_width, box_height) {
  ...
  .append('div')
    ...
    .style("top", (d) => (d.fy-box_height/2 + 45) + "px")
...
export default class CreateGraph extends React.Component {
  ...
  render() {
    ...
    let padding = 25, 
      ...
      legend_width=125,
      width = max_x_pos * radius * 5 , 
      height = max_y_pos * radius * 5 + padding;

    if (!this.state.full_toggle) {
      height = max_y_pos * radius * 3 + padding;
    }
    ...
    return (
      <div style={{position:'relative'}}>
        ...
        <p style={{"fontSize": "75%"}}> <span style={{"fontWeight": "bold", "fontStyle": "italic"}}> Bold, italicized</span> properties are required</p>
