...
class DataDictionaryPropertyTable extends React.Component {
  render() {
    ...
    const propertyKeysList = this.props.hideIsRequired
      ? Object.keys(this.props.properties)
      : Object.keys(this.props.properties).sort((k1, k2) => {
    ...
    const needHighlightSearchResult =
        ...
        <table className='data-dictionary-property-table__table'>
          ...
          <tbody>
            {propertyKeysList.map((propertyKey) => {
              ...
              const isRequired = this.props.requiredProperties.includes(
                propertyKey
              );
              return (
                <tr key={propertyKey}>
