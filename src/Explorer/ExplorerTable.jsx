...
class ExplorerTableComponent extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    getReduxStore().then((store) => {
      ...
      if (explorerState.moreData === 'RECEIVED') {
        ...
        const numberOfPages = filesList
          ? Math.ceil(filesList.length / nextProps.pageSize)
          : 0;
        const newPage =
          nextProps.originalPage + (this.props.page % this.props.pageCount);
        const page =
          numberOfPages > this.props.page % this.props.pageCount
            ? newPage
            : nextProps.originalPage + (numberOfPages - 1);
        this.setState({
          page,
  ...
  loadMoreNext() {
    this.setState({
      originalPageToSet: this.state.originalPage + this.props.pageCount,
    });
  ...
  static renderRow(user, projectAvail, file, i) {
    ...
    return (
      <tr key={i} className='explorer-table__table-row'>
        ...
        <td className='explorer-table__table-data explorer-table__table-data--column-2'>
          {file.format}
        </td>
        <td className='explorer-table__table-data explorer-table__table-data--column-3'>
          {filesize}
        </td>
        <td className='explorer-table__table-data explorer-table__table-data--column-4'>
          {file.category}
        </td>
      </tr>
    );
  ...
  render() {
    ...
    const filesList = this.props.filesList
      ? this.props.filesList.slice(
          startingPage * this.props.pageSize,
    ...
    if (this.state.pageSize > 0) {
      const numberOfPages = this.props.filesList
        ? this.props.filesList.length / this.state.pageSize
        : 0;
        ...
        <tbody>
          {filesList &&
            filesList.map((item, i) =>
              ExplorerTableComponent.renderRow(
                this.props.user,
                this.props.projectAvail,
                item,
                i
              )
            )}
          ...
          <tr className='explorer-table__table-row'>
            <td className='explorer-table__table-data explorer-table__table-data--foot-column-0'>
              {this.state.originalPage > 0 && (
                <button
                  className='explorer-table__arrow-button'
                  onClick={() => this.loadMorePrev()}
                >
            ...
            <td className='explorer-table__table-data explorer-table__table-data--foot-column-1'>
              {pages.map((item) => (
                <button
                  className={`explorer-table__page-button ${
                    item === this.state.page
                      ? 'explorer-table__page-button--active'
                      : ''
                  }`}
                  key={item}
