...
class LTNPCase extends HIVCohortFilterCase {
  ...
  updateSubjectClassifications = async () => {
    this.getBucketByKey().then((filtFollowups) => {
      const { subjectLTNP, subjectControl } = this.classifyAllSubjectLTNP(
        filtFollowups
      );
      this.setState({
        subjectLTNP,
  ...
  downloadLTNP = () => {
    const fileName = `ltnp-cohort-CD4-${this.state.CD4FromUser.toString()}-years-${this.state.numConsecutiveYearsFromUser.toString()}.json`;
  ...
  downloadControl = () => {
    const fileName = `control-cohort-cd4-${this.state.CD4FromUser.toString()}-years-${this.state.numConsecutiveYearsFromUser.toString()}.json`;
  ...
  checkReadyToCalculate = () => {
    ...
    const numConsecutiveYearsFromUser = this.numConsecutiveYearsInputRef.current
      .valueAsNumber;
    this.setState({
      CD4FromUser: CD4FromUser > 0 ? CD4FromUser : undefined,
      numConsecutiveYearsFromUser:
        numConsecutiveYearsFromUser > 0
          ? numConsecutiveYearsFromUser
          : undefined,
  ...
  showCount = (isLTNP) => {
    ...
    if (this.state.resultAlreadyCalculated) {
      if (isLTNP) {
        return this.state.subjectLTNP.length;
      }
          ...
          <form>
            ...
            <div className='hiv-cohort-filter__sidebar-input-label'>
              ...
              <span className='hiv-cohort-filter__value-highlight'>
                ...
                {this.state.numConsecutiveYearsFromUser === 1
                  ? 'year'
                  : 'years'}
              </span>
            </div>
        ...
        <div className='hiv-cohort-filter__main'>
          <div className='hiv-cohort-filter__main-wrapper'>
            <div className='hiv-cohort-filter__svg-wrapper'>
              ...
              <div
                id='download-control-cohort-overlay-LTNP'
                className='hiv-cohort-filter__overlay'
              >
                {
                  <React.Fragment>
