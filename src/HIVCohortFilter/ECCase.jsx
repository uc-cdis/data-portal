...
class ECCase extends HIVCohortFilterCase {
  ...
  filterFollowup = (subjectList, followupList) => {
    ...
    subjectList.forEach((item) => {
      ...
      for (let i = 0; i < followupList[subject].length; i += 1) {
        if (
          followupList[subject][i].visit_date <= item.convy ||
          followupList[subject][i].visit_date == null
        ) {
          // eslint-disable-next-line no-continue
          continue;
        } else if (
          (item.haarty === null && item.arty === null) ||
          (item.haarty === null &&
    ...
    const filtFollowups = Object.values(filtFollowup).filter(
      (x) => x.length !== 0
    );
  ...
  async getBucketByKey() {
    const subjectList = await Promise.all([this.getSubjectWithTime()]);
    const followupList = await Promise.all([this.getFollowupsBuckets()]);
  ...
  visitsMatchECWindowCriteria = (visitArray) => {
    ...
    for (let i = 0; i < visitArray.length; i += 1) {
      if (
        ...
        if (
          i === visitArray.length - 1 &&
          nSuper >= this.state.numConsecutiveVisitsFromUser
        ) {
          nEC += 1;
        ...
        visitArray[i].viral_load < this.state.spikeViralLoadFromUser &&
        nSuper > 0
      ) {
        nSpike += 1;
        if (nSpike > 1) {
  ...
  updateSubjectClassifications = async () => {
    this.getBucketByKey().then((filtFollowups) => {
      const { subjectEC, subjectControl } = this.classifyAllSubjectEC(
        filtFollowups
      );
      this.setState({
        subjectEC,
  ...
  makeCohortJSONFile = (subjectsIn) => {
    ...
    const blob = new Blob([JSON.stringify(annotatedObj, null, 2)], {
      type: 'text/json',
    });
    return blob;
  };
  ...
  downloadEC = () => {
    const fileName = `ec-cohort-suppressvload-${this.state.suppressViralLoadFromUser.toString()}-spikevload-${this.state.spikeViralLoadFromUser.toString()}-visits-${this.state.numConsecutiveVisitsFromUser.toString()}.json`;
  ...
  checkReadyToCalculate = () => {
    const suppressViralLoadFromUser = this.suppressViralLoadInputRef.current
      .valueAsNumber;
    const spikeViralLoadFromUser = this.spikeViralLoadInputRef.current
      .valueAsNumber;
    const numConsecutiveVisitsFromUser = this.numConsecutiveVisitsInputRef
      .current.valueAsNumber;
    this.setState({
      suppressViralLoadFromUser:
        suppressViralLoadFromUser > 0 ? suppressViralLoadFromUser : undefined,
      spikeViralLoadFromUser:
        spikeViralLoadFromUser > 0 ? spikeViralLoadFromUser : undefined,
      numConsecutiveVisitsFromUser:
        numConsecutiveVisitsFromUser > 0
          ? numConsecutiveVisitsFromUser
          : undefined,
      isReadyToCalculate:
        suppressViralLoadFromUser > 0 &&
  ...
  showCount = (isEC) => {
    if (this.state.inLoadingState) {
      return <Spinner />;
    }
    if (this.state.resultAlreadyCalculated) {
      if (isEC) {
        return this.state.subjectEC.length;
      }
    ...
    return (
      <React.Fragment>
        <div className='hiv-cohort-filter__ec-sidebar'>
          <form>
            <h2 className='hiv-cohort-filter__sidebar-title'>EC Classifier</h2>
            <h4 className='hiv-cohort-filter__sidebar-subtitle'>
              Customized Filters
            </h4>
            <div className='hiv-cohort-filter__sidebar-input-label'>
              Suppress Viral Load
              <span className='hiv-cohort-filter__value-highlight'>
                &nbsp; &lt; {this.state.suppressViralLoadFromUser || '__'}{' '}
                &nbsp;cp/mL
              </span>
            </div>
            ...
            <div className='hiv-cohort-filter__sidebar-input-label'>
              Spike Viral Load
              <span className='hiv-cohort-filter__value-highlight'>
                &nbsp; &lt; {this.state.spikeViralLoadFromUser || '__'}{' '}
                &nbsp;cp/mL
              </span>
            </div>
            <div className='hiv-cohort-filter__sidebar-input'>
            ...
            <div className='hiv-cohort-filter__sidebar-input-label'>
              ...
              <br />
              <span className='hiv-cohort-filter__value-highlight'>
                {this.state.numConsecutiveVisitsFromUser || '__'} &nbsp;
                {this.state.numConsecutiveVisitsFromUser === 1
                  ? 'visit'
                  : 'visits'}
              </span>
            </div>
            ...
            <div className='hiv-cohort-filter__button-group'>
              <Button
                ...
                enabled={
                  !this.state.inLoadingState && this.state.isReadyToCalculate
                }
        ...
        <div className='hiv-cohort-filter__main'>
          <div className='hiv-cohort-filter__main-wrapper'>
            <div className='hiv-cohort-filter__svg-wrapper' id='EC-svg-wrapper'>
              ...
              <div
                id='download-control-cohort-overlay-EC'
                className='hiv-cohort-filter__overlay'
              >
                {
                  <React.Fragment>
