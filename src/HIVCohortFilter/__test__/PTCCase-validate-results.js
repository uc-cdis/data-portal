...
console.log(
  `Validating PTC results with argument ${viralLoadUpperBound}, ${numConsecutiveMonthsOnHAART}`
);
...
function checkIfConsecutiveFollowUpsLessThanViralLoadLimit(
  ...
  followUps.forEach((fu) => {
    if (fu.viral_load >= viralLoadUpperBound) {
      console.error(
        `Error with subject ${subjectID}: the follow up ${fu.submitter_id}'s viral_load = ${fu.viral_load} >= ${viralLoadUpperBound}`
      );
    }
    if (fu.thrpyv !== 'HAART') {
      console.error(
        `Error with subject ${subjectID}: the follow up ${fu.submitter_id}'s thrpyv = ${fu.thrpyv} !== HAART`
      );
    }
  });
}
...
function confirmPatientIsPTC(subjectObj) {
  ...
  const startFollowUpID =
    subjectObj.consecutive_haart_treatments_begin_at_followup;
  ...
  for (let i = 0; i < followUps.length; i += 1) {
    ...
    if (followUp.submitter_id === startFollowUpID) {
      ...
      if (windowSize > numConsecutiveMonthsOnHAART * 2) {
        ...
        console.log(
          `Warning: subject ${subjectID}'s sliding window is of size ${windowSize} > ${numConsecutiveMonthsOnHAART} months`
        );
      }
      ...
      if (nextFollowUp.thrpyv === 'HAART') {
        console.error(
          `Error with subject ${subjectID}: next follow up ${nextFollowUp.submitter_id}'s thrpyv = HAART`
        );
      }
      if (nextFollowUp.viral_load >= viralLoadUpperBound) {
        console.error(
          `Error with subject ${subjectID}: next follow up ${nextFollowUp.submitter_id}'s viral_load = ${nextFollowUp.viral_load} >= ${viralLoadUpperBound}`
        );
      }
      return;
    }
  ...
  console.error(
    `Error with subject ${subjectID}: cannot find followup with submitter_id = ${startFollowUpID}`
  );
}
